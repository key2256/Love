...
export const InquiryForm: React.FC<InquiryFormProps> = ({ quotation, onClose, isInline = false }) => {
  ...
  const SuccessContent = (
    <motion.div 
      ...
      className={`bg-white rounded-[40px] p-12 max-w-md w-full text-center ${!isInline ? 'shadow-2xl' : ''}`}
    >
  ...
  const FormContent = (
    <motion.div 
      initial={isInline ? {} : { opacity: 0, y: 20 }}
      animate={isInline ? {} : { opacity: 1, y: 0 }}
      className={`bg-white rounded-[40px] w-full max-w-2xl overflow-hidden ${!isInline ? 'shadow-2xl my-8' : ''}`}
    >
