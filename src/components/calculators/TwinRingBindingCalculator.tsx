...
import { Layers, Settings2, ShoppingCart, FileText, Scissors, CheckCircle2, BookOpen, Palette } from 'lucide-react';
...
interface TwinRingBindingCalculatorProps {
  product: Product;
  quantity: number;
...
export const TwinRingBindingCalculator: React.FC<TwinRingBindingCalculatorProps> = ({
  product,
  quantity,
  setQuantity,
  selectedOptions,
              ...
              return (
                ...
                >
                  <div className={`${isSelected ? 'text-white' : 'text-zinc-300'}`}>
                    {Icon ? <Icon className="w-6 h-6" /> : (option.name.includes('색상') ? <Palette className="w-6 h-6" /> : <BookOpen className="w-6 h-6" />)}
                  </div>
  ...
  const sections = [
    ...
    },
    {
      id: 'material',
      title: '재질 및 옵션',
      description: '표지와 내지의 용지 및 트윈링 색상을 선택하세요.',
      icon: Layers,
      children: (
        <div className="space-y-8">
      ...
      children: (
        <div className="space-y-8">
          ...
          <InfoCard 
            title="트윈링제본 안내"
            content={[
              "넘김이 부드럽고 정돈된 느낌을 주는 제본 방식입니다.",
              "기관 제출용 자료나 깔끔한 포트폴리오에 적합합니다.",
              "다양한 링 색상으로 전문적인 분위기를 연출할 수 있습니다."
            ]}
          />
        </div>
      )
